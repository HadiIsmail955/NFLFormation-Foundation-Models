from src.train.train_classifier_v1_0 import train_phase
from src.test.test_classifier_v1_0 import test_phase
from src.utils.experiment_logger import ExperimentLogger

def main():
    cfg = {
        "experiment_name": "nfl_formation_classifier_v1_0",
        "experiment_description": "Training classifier model for NFL formation detection using SAM backbone. Version 1.0 with adjusted hyperparameters.",
        "experiment_goals": [
            "Achieve high accuracy in classifying NFL formations from images.",
            "Utilize SAM backbone for improved feature extraction.",
            "Optimize training process with effective hyperparameter tuning."
        ],
        "seed": 42,
        "data_root": "dataSet",
        "train_coco_file": "dataSet/splits/train.json",
        "test_coco_file": "dataSet/splits/test.json",
        "seg_from_ckpt": None,
        "img_size": 1024,
        "val_split": 0.1,
        "batch_size": 4,
        "num_workers": 4,
        "sam_type": "vit_h",
        "sam_ckpt_dir": "src/model/models",
        "unfreeze_last_blocks": 0,
        "epochs": 50,
        "lr": 1e-3,
        "weight_decay": 1e-4,
        "grad_clip": 1.0,
        "patience": 10,
        "lr_decay_factor": 0.5,
        "min_lr": 1e-7,
        "early_stopping": True,
        "dino_type": "vit_b",
        "dino_ckpt_dir": "src/model/models",
        "num_classes": 15,
        "mask_mode": "soft",
    }
    
    train=True
    logger = ExperimentLogger(exp_name="seg_phase_v1-0")
    logger.save_config(cfg)
    if train:
        train_phase(cfg, logger)
    test_phase(cfg, logger)


if __name__ == "__main__":
    main()
...
def train_phase(cfg, logger):
    ...
    model = SAMFormationModel(
        ...
        trainable_decoder=cfg["trainable_decoder"],
    ).to(device)
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        for batch in train_bar:
            ...
            points_xy = batch["points_xy"].to(device, non_blocking=True) 
            points_label = batch["points_label"].to(device, non_blocking=True) 
        ...
        with torch.no_grad():
            for batch in val_bar:
                ...
                with autocast(device_type="cuda", enabled=amp_enabled):
                    logits = model(x, points_xy, points_label, valid_mask=valid_mask)
                    loss = loss_fn(logits, y)
